<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Content Language Lines
    |--------------------------------------------------------------------------
    */
    'header' => [
        'home' => 'Accueil',
        'quotes' => 'Citations',
        'new' => 'Ajouter',
        'about' => 'À propos',
        'back' => 'Retour',
        'search' => 'Chercher une citation'
    ],

    'home' => [
        'subtitle' => 'Hors contexte, sans aucun sérieux.',
        'latest' => [
            'title' => 'Récentes',
            'more' => 'Voir toutes les citations'
        ],
        'popular' => [
            'title' => 'Citations à la une',
            'more' => 'En voir plus'
        ],
    ],
    
    'results' => [
        'title' => '{0} Aucun résultat...|{1} Résultat de recherche|[2,*] Résultats de recherche',
    ],
    
    'add' => [
        'title' => 'Ajouter une citation',
        'labels' => [
            'group' => 'Groupe',
            'content' => 'Citation',
            'author' => 'Auteur'
        ],
        'inputs' => [
            'none' => 'Aucun',
            'content' => 'I have a quote',
            'author' => 'Martin Luther King',
            'submit' => 'Ajouter'
        ]
    ],

    'edit' => [
        'title' => 'Éditer la citation',
        'labels' => [
            'group' => 'Groupe',
            'content' => 'Citation',
            'author' => 'Auteur'
        ],
        'inputs' => [
            'none' => 'Aucun',
            'content' => 'C\'est vide!',
            'author' => 'C\'est vide!',
            'submit' => 'Enregistrer',
            'delete' => 'Supprimer'
        ]
    ],

    'show' => [
        'edit' => 'Éditer la citation'
    ],

    'index' => [
        'title' => 'Toutes les citations, cliquez les!',
    ],

    'about' => [
        'title' => 'Pourquoi, quoi donc?',
        'why' => [
            'title' => 'Pourquoi?',
            'content' => [
                "Pour regrouper tous types de citations, parce qu'une citation n'est pas nécessairement une phrase philosophique sacrée."
            ]
        ],
        'how' => [
            'title' => 'Comment?',
            'content' => [
                'À un moment, une phrase est dite. Mettez cette phrase hors de son contexte et vous obtiendrez une citation. Quoted.'
            ]
        ],
        'what' => [
            'title' => 'C\'est quoi?',
            'content' => [
                "\"Quoted.\" est un site ou sont regroupées les meilleures phrases hors-contexte de tout le monde. Comme tout le monde peut y rajouter sa citation, le site et son propriétaire ne pourraient être tenu du contenu présent sur le site.",
                "Vous verrez peut-être du contenu politque, limite ou controversé, des paroles d'humour noir, des citations philosophiques et officielles. Gardez toutefois à l'esprit que le site ne supporte aucun contenu raciste, sexiste ou haineux.",
                "Lorsque la tendance actuelle est à la polémique de tout sujet, il faut laisser une place à la liberté d'expression et rester ouvert d'esprit. Évidemment, si vous pensez un contenu inapproprié, vous pouvez contacter contact@proust.dev, ou supprimer par vous même le contenu - qui ne serait pas une phrase hors contexte borderline - réellement haineux, raciste ou sexiste."
            ]
        ],
    ],

    'footer' => [
        'credit' => 'Développé avec passion par Arthaud Proust',
        'right' => 'Tous droits réservés',
        'more' => 'À propos et plus'
    ]
];
