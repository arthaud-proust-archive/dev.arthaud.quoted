...
$(()=>{
    ...
    $('form[id^="edit"] input[type="submit"]').on('click', function() {
        localStorage.setItem('nextEdit', Date.now()+1000*30);
    })

    $('form[id^="delete"] input[type="submit"]').on('click', function(e) {
        e.preventDefault()
        let confirmation = confirm('Delete this quote? Please Don\'t do this if you aren\'t the author or if you don\'t have his agreement');
        if(confirmation) {
            localStorage.setItem('nextEdit', Date.now()+1000*30);
    ...
    if(localStorage.getItem('nextEdit')==undefined || Date.now() > localStorage.getItem('nextEdit')) {
        $('#toEdit').show();
    }
